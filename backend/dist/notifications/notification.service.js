...
export async function updateNotificationSettings(userId, settingsInput) {
    const { data, error } = await supabase
        .from("notification_settings")
        .upsert({
        user_id: userId,
...
export async function createNotification(input) {
    ...
    if (input.type === "like" && !settings.likes_enabled)
        return null;
    if (input.type === "comment" && !settings.comments_enabled)
        return null;
    if (input.type === "reply" && !settings.comments_enabled)
        return null;
    if (input.type === "follow" && !settings.follows_enabled)
        return null;
    if (input.type === "mention" && !settings.mentions_enabled)
        return null;
